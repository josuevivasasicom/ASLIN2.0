...
class Siniestros
{
    ...
    public static function getNumPolizaValores($timerst)
    { //?busca los ids asignados a polizas y los amarra con el timerst o folio ID del siniestro por siniestro,
    ...
    {
        $sql = "SELECT (select valor from config_campos where estado = 'A' and id = status) status from siniestros_areas where timerst = '$timerst'  and area = '$areaID'  limit 1";
        $query = Database::ExeDoIt($sql, false);
        if ($query[0]->num_rows) {
        ...
        $sql = "select (select valor from config_campos where estado = 'A' and  id = status) status from siniestros where timerst = '$timerst' limit 1 ";
        $query = Database::ExeDoIt($sql, false);
    ...
    {
        $sql = "SELECT (select valor from config_campos where estado = 'A' and id = calificacion) calificacion from siniestros_areas where timerst = '$timerst'  and area = '$areaID'  limit 1";
        $query = Database::ExeDoIt($sql, false);
        if ($query[0]->num_rows) {
        ...
        //en caso de no existir status, el flujo sigue.
        $sql = "select (select valor from config_campos where estado = 'A' and id = calificacion ) calificacion from siniestros where estado1 = 'A' and  timerst = '$timerst' limit 1 ";
        $query = Database::ExeDoIt($sql, false);
    ...
    public static function nuevaEntradaBitacora($r, $redirect = true)
    {
        ...
        $usuario = $_SESSION['id'];
        $area = $r['area'];
        if (!isset($r['fechaActividad'])) {
            $r['fechaActividad'] = date("Y-m-d");
        }
    ...
    /**************************************
     * ELIMINAR LOS ARCHIVOS DEL SINIESTRO Y SUS VERSIONES                 [PESTAÑA DOCUMENTACIÓN]
     *************************************/

    public static function eliminarArchivo($id, $timerst)
    {

        $sql = 'UPDATE `siniestros_files` SET `estatus` = 0 WHERE id = ' . $id . ';';

        $query = Database::ExeDoIt($sql);

        if ($query) {
            $message = 'Eliminaste el archivo con id: ' . $id . ' / ' . $timerst;
        } else {
    ...
    {
        $sql="SELECT s.*, s.id as id, s.version version,LOWER(c1.valor) c1,LOWER(c2.valor) c2,LOWER(c3.valor) c3 FROM `siniestros_files` s
        ...
        where s.timerst = '".$timerst."' 
        and s.estatus = '1'
        and (s.area='$areaId' or s.area=(select id from areas where area='$areaId' limit 1 ) ) 
        and version = (select max(version) from `siniestros_files` as s1
        where s1.timerst = '".$timerst."' and s.nombre = s1.nombre and s.area = s1.area) order by fecha desc";
        //echo $sql;
        /*$sql = "SELECT s.*, s.id as id, s.version, LOWER(c1.valor) c1, LOWER(c2.valor) c2, LOWER(c3.valor) c3
        FROM `siniestros_files` s 
        LEFT JOIN config_files c1 ON c1.id = id_config_files 
        LEFT JOIN config_files c2 ON c2.id = c1.parent 
        LEFT JOIN config_files c3 ON c3.id = c2.parent 
        WHERE s.timerst = '" . $timerst . "' AND estatus = 1
        AND (s.area='$areaId' OR s.area=(SELECT id FROM areas WHERE area='$areaId' LIMIT 1))
        and s.version IN ( SELECT max(a.version) FROM `siniestros_files` a WHERE a.timerst = s.timerst and a.nombre=s.nombre) 
        group by nombre
        ORDER BY fecha DESC";*/
        /*$sql = "SELECT s.*, s.id as id, s.version, LOWER(c1.valor) c1, LOWER(c2.valor) c2, LOWER(c3.valor) c3 
        FROM `siniestros_files` s
        ...
        AND estatus = 1
        AND (s.area='$areaId' OR s.area=(SELECT id FROM areas WHERE area='$areaId' LIMIT 1)
        AND (s.nombre, s.version) IN (
          SELECT nombre, MAX(version) 
          FROM `siniestros_files` 
          WHERE timerst = '" . $timerst . "'
          GROUP BY nombre
        ) )
        ORDER BY fecha DESC";*/
        //echo $sql;
        ...
        // LEFT JOIN config_files c2 on c2.id = c1.parent
        // LEFT JOIN config_files c3 on c3.id = c2.parent
        // where `status` = 1 and s.timerst = '".$timerst."' and (s.area='$areaId' or s.area=(select id from areas where area='$areaId' limit 1 ) ) 
        // and version = (select max(version) from `siniestros_files` as s1
     ...
     *************************************/
    public static function guardarPrimeraAtencion($timerst, $data, $edit = false)
    { //guarda primera atencion desde todos los siniestros

        //obtener version de formato.
        $sqlv = "select version from siniestros_files where timerst='" . $data['timerst'] . "' and nombre='primeraAtencion' and estatus = 1 and area='" . $data['areaID'] . "' order by id desc limit 1;";
        ...
        if ($query)
            print "primera atencion guardada OK";
    }
    //ver primera atención  
    public static function verPrimeraAtencion($timerst, $area, $version = '')
    { //muestra los datos de la primera atencion 
        if ($version != '') {
            $version = ' and spa.rev=' . $version;
        }
        
        $area_aux ="";
        if (strcmp($area,"ServidoresPúblicos") == 0){
        $area_aux = "Servidores Públicos";
        }else{
            $area_aux = $area;
        }
        $sql = "select spa.*, 
        (select concat(u.nombre,' ',u.paterno) autor from usuarios u where u.id = spa.autor limit 1) autor,
        (select concat(nombre,' ',apellidoP,' ',apellidoM) from siniestros s where s.estado1 = 'A' and s.timerst=spa.timerst) asegurado
         from siniestros_primera_atencion spa where spa.timerst = '" . $timerst . "' and spa.area = (select id from areas where area = '" . $area_aux . "') $version  order by id desc limit 1";
        $query = Database::ExeDoIt($sql, false);
        $data = Model::many_assoc($query[0])[0];
        $data['area'] = $area_aux;
    ...
    { //guarda informe preliminar desde todos los siniestros
        ...
        if ($query)
            print "informe preliminar guardada OK";
    }

    public static function segundaPartePreliminar($dataForm)
    {
        ...
        if (strlen($data['fecha1raAtencion']) != 10) {
            $data['fecha1raAtencion'] = date("Y-m-d");
        }
...
Observaciones:
 ...
 [form] => noPoliza=1-66-7010904-3-0%20%2F%20&asegurado=RICARDO%20JAIME%20ROMERO&fechaOcurrido=2022-07-07&tercero=tercero&causa=causa&lugar=lugar&ciudad=ciudad&agente=agente&practica=practivca&vigenciaInicio=2022-07-08&vigenciaFin=2022-07-22&fechaReporte=2022-07-01&fechaAsignacion=2022-07-09&fecha1raAtencion=2022-07-13&autoridad=CONAMED&noExpediente=expediente&etapa=etapa&preliminarHechos=%20&causaProxima=causa%20&reclamacion=reclamacion&fundamento=fundamento&sumAsegurada=suma&estimacion=estimacion&importeReclamado=importe&reservaRecomendada=reserva&preliminarObservaciones=Observaciones%3A%20%20
    ...
    public static function verInformePreliminar($timerst, $area, $version = '')
    { //muestra los datos de la primera atencion 
        if ($version != '') {
            $version = ' and sip.rev=' . $version;
        }
        $area_aux ="";
        if (strcmp($area,"ServidoresPúblicos") == 0){
        $area_aux = "Servidores Públicos";
        ...
        (select concat(nombre,' ',apellidoP,' ',apellidoM) from siniestros s  where s.estado1 = 'A' and s.timerst='" . $timerst . "' limit 1) asegurado,
        (select sin.numSiniestro from siniestros sin where sin.estado1 = 'A' and  sin.timerst='" . $timerst . "' limit 1) numSiniestro,
        (select sa.calificacion from siniestros_areas sa where sa.timerst= '" . $timerst . "' and sa.area= (select id from areas where area = '" . $area_aux . "' limit 1) limit 1) calificacion
        from siniestros_informe_preliminar sip where sip.timerst = '" . $timerst . "' and sip.area = (select id from areas where area = '" . $area_aux . "' limit 1) $version order by id desc limit 1;";
        $query = Database::ExeDoIt($sql);
        ...
        (select id from areas where area = '" . $area_aux . "') 
        ...
        if ($data['calificacion'] == '') { /*********** */
            $sql = "select sa.calificacion from siniestros sa where sa.estado1 = 'A' and  sa.timerst= '" . $timerst . "' limit 1";
            $query = Database::ExeDoIt($sql);
    ...
    public static function guardarInformeActualizacion($timerst, $data, $edit = false)
    { //guarda primera atencion desde todos los siniestros
        ...
        if ($query)
            print "informe actualizacion guardada OK";
    }
    //ver informe actualización 
    public static function verInformeActualizacion($timerst, $area, $version = '')
    { //muestra los datos de la primera atencion 
        if ($version != '') {
            $version = ' and sia.rev=' . $version;
        }
        $area_aux ="";
        if (strcmp($area,"ServidoresPúblicos") == 0){
        $area_aux = "Servidores Públicos";
        ...
        (select concat(nombre,' ',apellidoP,' ',apellidoM) from siniestros s where s.estado1='A' and s.timerst=sia.timerst) asegurado
         from siniestros_informe_actualizacion sia where sia.timerst = '" . $timerst . "' and sia.area = (select id from areas where area = '" . $area_aux . "') $version order by id desc limit 1";
    ...
    { //guarda informe de cancelacion desde todos los siniestros
        ...
        if ($query)
            print "Informe Cancelación guardada OK";
    }
    //ver informe de Cancelacion
    public static function verInformeCancelacion($timerst, $area, $version = '')
    { //muestra los datos de la primera atencion 
        if ($version != '') {
            $version = ' and sic.rev=' . $version;
        }
        $area_aux ="";
        if (strcmp($area,"ServidoresPúblicos") == 0){
        $area_aux = "Servidores Públicos";
        ...
        (select concat(nombre,' ',apellidoP,' ',apellidoM) from siniestros s where s.estado1 = 'A' and s.timerst=sic.timerst) asegurado
        from siniestros_informe_cancelacion sic where sic.timerst = '" . $timerst . "' and sic.area = (select id from areas where area = '" . $area_aux . "') $version order by id desc limit 1;";
        $query = Database::ExeDoIt($sql, false);
        if ($query[0]->num_rows > 0)
        $data = Model::many_assoc($query[0])[0];
        $data['area'] = $area_aux;
        
        //echo $data["informe_cancelacion"];
        //die();
        return $data;
        //!debe hacerse un URLdecode al retornar los datos para verlos correctamente. ***HECHO!!
    }
    ...
    *************************************/
    ...
    {
        $sql = "SET lc_time_names = 'es_ES';";
        $query = Database::ExeDoIt($sql); // esencial para poner en español
     ...
     *************************************/
    public static function countAllareasOfSiniestro($timerst)
    { // utilizado para jtabla siniestros/areas
    ...
    public static function getAllAreasOfSiniestro($timerst)
    { // utilizado para jtabla siniestros/areas
    ...
    public static function getAllAbogadosOnSiniestro($timerst)
    { //obtiene todos  los abogados asignados al siniestro 
    ...
    public static function datosGraficasIndexCalificaciones()
    {
        ...
        //retorna el array de json`s con los datos de cada proveniente para contar todos los siniestros y generar las graficas.
        $sql = "SELECT count(s.id)cantidad, (select valor from config_campos cc where cc.id = s.calificacion and cc.estado = 'A') calificacion  FROM `siniestros` s WHERE estado1='A' GROUP by calificacion ";
        $query = Database::exeDoIt($sql);
        ...
        foreach ($calificaciones as $d) {
            ...
            $labels[] = mb_convert_case($d['calificacion'], MB_CASE_UPPER, "UTF-8");
    ...
    public static function datosGraficasIndexStatus()
    {
        ...
        foreach ($status as $d) {
            //armar el json que se entrega al js
            $labels[] = mb_convert_case($d['status'], MB_CASE_UPPER, "UTF-8");
            $cantidades[] = strtoupper($d['cantidad']);
            // 'data'=> [10, 11, 15, 10, 3, 14, 15, 11, 6, 0]
        }
        $data = [$labels, $cantidades];
    ...
    public static function countTodosSiniestrosCancelados($prov = '')
    { // utilizado para jtabla siniestros
        $sql = "select count(id) siniestros from siniestros where estado1='A' ";
        $sql .= " and status = '169'"; // 	166 en estatus es igual a CANCELADOS tenia 175
        if ($prov != '') {
    ...
    public static function countTodosSiniestrosVigentes($prov = '')
    { // utilizado para jtabla siniestros
        $sql = "select count(id) siniestros from siniestros where estado1 = 'A' ";
        $sql .= " and status = '167'"; // 	166 en estatus es igual a VIGENTE tenia 169
        if ($prov != '') {
            $sql .= " and f_numProv = (select id from config_prov where proveniente = '" . $prov . "' limit 1)";
        }
        $query = Database::exeDoIt($sql);
    ...
    public static function countTodosSiniestrosProcesoCancelacion($prov = '')
    { // utilizado para jtabla siniestros
        $sql = "select count(id) siniestros from siniestros where estado1='A' ";
        $sql .= " and status = '175'"; // 	168 en estatus es igual a PROCESO CANCELACIÓN tenia 169
        if ($prov != '') {
            $sql .= " and f_numProv = (select id from config_prov where proveniente = '" . $prov . "' limit 1)";
        }
        $query = Database::exeDoIt($sql, false);
    ...
    public static function countTodosSiniestros($prov = '', $statusS = '', $area = '', $calificacion = '', $asignados = 'false', $abogados = 0)
    { // utilizado para jtabla siniestros  //?OPTIMIZADO PARA VARIOS USUARIOS
        $sql = '';
        if ($area != '') {
        ...
        if ($prov != '' and $statusS != '') {
            ...
            inner JOIN siniestros_areas sa on s.timerst = sa.timerst
            LEFT JOIN siniestros_usuarios su on s.timerst = su.timerst
            where s.estado1='A' and " . $sqlarea . " 1 " . $sqlcalifica . '  ' . $sqlasignados;
            ...
            where s.estado1='A' and " . $sqlarea;
            if ($prov != '')
                $sql .= " s.f_numProv = (select id from config_prov where proveniente = '" . $prov . "' ) and";
            if ($statusS != '')
                $sql .= " s.status = '$statusS' and ";
    ...
    public static function countBusquedaTodosSiniestros($prov = '', $statusS = '', $area = '', $calificacion = '', $asignados = 'false', $asegurado = '')
    { // utilizado para jtabla siniestros  //?OPTIMIZADO PARA VARIOS USUARIOS
        $sql = '';
        if ($area != '') {
        ...
        if ($prov != '' and $statusS != '') {
            ...
            inner JOIN siniestros_areas sa on s.timerst = sa.timerst
            LEFT JOIN siniestros_usuarios su on s.timerst = su.timerst
            where s.estado1='A' and concat(s.nombre,' ',s.apellidoP,' ',s.apellidoM) like  '%" . $asegurado . "%' " . $sqlarea . ' ' . $sqlcalifica . ' ' . $sqlasignados;
        } else if (($prov == '' and $statusS != '') or ($prov != '' and $statusS == '')) {
            $sql .= "select  COUNT(DISTINCT s.timerst) siniestros from siniestros s
            inner JOIN siniestros_areas sa on s.timerst = sa.timerst
            LEFT JOIN siniestros_usuarios su on s.timerst = su.timerst
            where s.estado1='A' and concat(s.nombre,' ',s.apellidoP,' ',s.apellidoM) like  '%" . $asegurado . "%' ";
            $sql .= $sqlarea;
            if ($prov != '')
                $sql .= " and s.f_numProv = (select id from config_prov where proveniente = '" . $prov . "' ) and";
            if ($statusS != '')
                $sql .= " and s.status = '$statusS' and ";
            $sql .= " 1 " . $sqlcalifica . ' ' . $sqlasignados;
        }
    ...
    public static function getBusquedaTodosSiniestros($orden, $limitInf, $limitSup, $_prov = '', $_status = '', $area = '', $calificacion = '', $asignados = false, $asegurado = '')
    { // utilizado para jtabla siniestros admin
        $sql = "SELECT s.id,concat(s.nombre,' ',s.apellidoP,' ',s.apellidoM) nombre, s.timerst, s.activo, concat(f_numProv,'-',LPAD(f_consecutivo,3,'0'),
        '-',SUBSTRING(f_year, 3)) folio, SUBSTRING(UPPER(proveniente),6) proveniente, 
        c.valor as institucion, cel,casa,oficina, fechaReporte,s.estado estado,ciudad,mail,formaContacto,descripcionHechos,numReporte,NumPoliza,numSiniestro,vigencia1,vigencia2,cs.valor calificacion,cc.valor autoridad,css.valor `status` ,
        fechaAsignacion,if(pa.primera_atencion<>'','si','no') as primera_atencion, if(ip.informe_preliminar<>'','si','no') as informe_preliminar,if(ic.informe_cancelacion<>'','si','no') as informe_cancelacion, fechaCaptura 
        ...
        $sql .= " LEFT JOIN config_campos c on s.institucion = c.id  and c.estado = 'A' ";
        $sql .= " LEFT JOIN config_campos cc on s.autoridad = cc.id  and cc.estado = 'A' ";
        $sql .= " LEFT JOIN config_campos cs on s.calificacion= cs.id and cs.estado = 'A' ";
        $sql .= " LEFT JOIN config_campos css on s.status= css.id  and css.estado = 'A' ";
        $sql .= " WHERE ";
        if ($area == '') {
            $sql .= "  1 ";
        } else {
        ...
        //GROUP BY s.timerst
        $sql .= ' and s.estado1="A" GROUP BY s.timerst ' . $orden . " limit " . $limitInf . "," . $limitSup . "  ";
    ...
    public static function countBusquedaNumSiniestros($prov = '', $statusS = '', $area = '', $calificacion = '', $asignados = 'false', $NS = '')
    { // utilizado para jtabla siniestros  //?OPTIMIZADO PARA VARIOS USUARIOS
        $sql = '';
        if ($area != '') {
            $sqlarea = " and sa.area ='" . $area . "'  ";
        ...
        if ($prov != '' and $statusS != '') {
            ...
            inner JOIN siniestros_areas sa on s.timerst = sa.timerst
            LEFT JOIN siniestros_usuarios su on s.timerst = su.timerst
            where s.estado1='A' and  numSiniestro like  '%" . $NS . "%' " . $sqlarea . ' ' . $sqlcalifica . ' ' . $sqlasignados;
        } else if (($prov == '' and $statusS != '') or ($prov != '' and $statusS == '')) {
            $sql .= "select  COUNT(DISTINCT s.timerst) siniestros from siniestros s
            inner JOIN siniestros_areas sa on s.timerst = sa.timerst
            LEFT JOIN siniestros_usuarios su on s.timerst = su.timerst
            where s.estado1='A' and numSiniestro like  '%" . $NS . "%' ";
            $sql .= $sqlarea;
            if ($prov != '')
                $sql .= " and s.f_numProv = (select id from config_prov where proveniente = '" . $prov . "' ) and";
            if ($statusS != '')
    ...
    public static function getBusquedaNumSiniestros($orden, $limitInf, $limitSup, $_prov = '', $_status = '', $area = '', $calificacion = '', $asignados = false, $NS = '')
    { // utilizado para jtabla siniestros admin
    ...
    public static function getTodosSiniestros($orden, $limitInf, $limitSup, $_prov = '', $_status = '', $area='', $calificacion = '', $asignados = false, $abogados = 0)
    { // utilizado para jtabla siniestros admin
        $sql = "SELECT s.id,concat(s.nombre,' ',s.apellidoP,' ',s.apellidoM) nombre, s.timerst, s.activo, concat(f_numProv,'-',LPAD(f_consecutivo,3,'0'),'-',SUBSTRING(f_year, 3)) folio, SUBSTRING(UPPER(proveniente),6) proveniente, c.valor as institucion, cel,casa,oficina, fechaReporte, s.estado as estado,ciudad,mail,formaContacto,descripcionHechos,numReporte,NumPoliza,numSiniestro,vigencia1,vigencia2,cs.valor calificacion,cc.valor autoridad,css.valor `status` ,fechaAsignacion,if(pa.primera_atencion<>'','si','no') as primera_atencion, if(ip.informe_preliminar<>'','si','no') as informe_preliminar,if(ic.informe_cancelacion<>'','si','no') as informe_cancelacion, fechaCaptura 
        ,su.usuario idUserAsign , su.estatus statusUserAsign
        FROM siniestros as s "; //siniestros
    ...
    public static function getTodosSiniestrosOLD($orden, $limitInf, $limitSup, $_prov = '', $_status = '', $area = '', $calificacion = '', $asignados = false)
    { // utilizado para jtabla siniestros admin
        $sql = "SELECT s.id,concat(s.nombre,' ',s.apellidoP,' ',s.apellidoM) nombre, s.timerst, s.activo, concat(f_numProv,'-',LPAD(f_consecutivo,3,'0'),'-',SUBSTRING(f_year, 3)) folio, SUBSTRING(UPPER(proveniente),6) proveniente, c.valor as institucion, cel,casa,oficina, fechaReporte,s.estado as estado,ciudad,mail,formaContacto,descripcionHechos,numReporte,NumPoliza,numSiniestro,vigencia1,vigencia2,cs.valor calificacion,cc.valor autoridad,css.valor `status` ,fechaAsignacion,if(pa.primera_atencion<>'','si','no') as primera_atencion, if(ip.informe_preliminar<>'','si','no') as informe_preliminar,if(ic.informe_cancelacion<>'','si','no') as informe_cancelacion, fechaCaptura 
        ,su.usuario idUserAsign , su.estatus statusUserAsign
        FROM siniestros as s "; //siniestros
        ...
        if ($_prov != '' && $_status != '') { // con status y prov
            ...
            core::preprint($_status, 'status no se cumple anda');
            exit();
        }
        if ($asignados == 'true') {
            $sql .= ' and  su.usuario = ' . $_SESSION['id'];
    ...
    { // utilizado para jtabla siniestros admin
        ...
        //GROUP BY s.timerst        
        //$sql.=' and s.estado1="A" GROUP BY s.timerst '. $orden."   ";  //limit ".$limitInf.",". $limitSup . "
        $sql .= " and s.estado1='A' GROUP BY s.timerst order by concat(SUBSTRING(f_year, 3),'-', LPAD(f_consecutivo,3,'0'),'-', f_numProv)  desc   ";
        //echo $sql;
        $query = Database::ExeDoIt($sql, false); //!REPARAR ESTA CONSULTA SQL
        // core::preprint($_status,'status');exit();
    ...
    // DESCARGAR EXCEL gmx
    public static function getTodosSiniestrosDownloadExcelGMX($orden, $limitInf, $limitSup, $_prov = '', $_status = '', $area = '', $calificacion = '', $asignados = false, $abogados = 0)
    { // utilizado para jtabla siniestros admin
        $sql = "SELECT 
        ...
        $sql .= " LEFT JOIN siniestros_informe_preliminar as ip on s.timerst = ip.timerst ";
        $sql .= " LEFT JOIN siniestros_informe_cancelacion as ic on s.timerst = ic.timerst ";
        $sql .= " LEFT JOIN siniestros_polizas as sp on s.numPoliza = sp.id ";
        $sql .= " LEFT JOIN config_campos c on s.institucion = c.id  and c.estado = 'A' ";
        $sql .= " LEFT JOIN config_campos cc on s.autoridad = cc.id  and cc.estado = 'A' ";
        $sql .= " LEFT JOIN config_campos cs on s.calificacion= cs.id and cs.estado = 'A' ";
        $sql .= " LEFT JOIN config_campos css on s.status= css.id  and css.estado = 'A' ";
    ...
    public static function getTodasNotificacionesSiniestros()
    { //utilizado por notificaciones
        ...
        foreach ($data as $dato) {
            ...
            $query = Database::ExeDoIt($sql2, false); //!REPARAR ESTA CONSULTA SQL
            //echo $sql; die();
            // core::preprint($_status,'status');exit();
            $data2 = Model::many_assoc($query[0]);
            //echo "*" . count($data2) . "*<br>";
            if (count($data2) > 1) {
                $detalle[$x] = "";
        ...
        where estado1 = 'A'";
        $query_a = Database::ExeDoIt($sql_a, false); //!REPARAR ESTA CONSULTA SQL
        $data_a = Model::many_assoc($query_a[0]);
        //$x=0;
        //echo "**************************$x**************************<br>";
        foreach ($data_a as $dato_a) {
            ...
            $query_b = Database::ExeDoIt($sql_b, false); //!REPARAR ESTA CONSULTA SQL
            //echo $sql; die();
            // core::preprint($_status,'status');exit();
            $data_b = Model::many_assoc($query_b[0]);
        ...
        where estado1 = 'A'";
        $query_c = Database::ExeDoIt($sql_c, false); //!REPARAR ESTA CONSULTA SQL
        $data_c = Model::many_assoc($query_c[0]);
        //$x=0;
        //echo "**************************$x**************************<br>";
        foreach ($data_c as $dato_c) {
            ...
            $query_d = Database::ExeDoIt($sql_d, false); //!REPARAR ESTA CONSULTA SQL
            //echo $sql; die();
            // core::preprint($_status,'status');exit();
            $data_d = Model::many_assoc($query_d[0]);
        ...
        $sql_e = "select distinct NumPoliza from siniestros s
            where estado1 = 'A'";
        $query_e = Database::ExeDoIt($sql_e, false); //!REPARAR ESTA CONSULTA SQL
        $data_e = Model::many_assoc($query_e[0]);
        //$x=0;
        //echo "**************************$x**************************<br>";
        ...
        group by NumPoliza, timerst";
            ...
            $query_f = Database::ExeDoIt($sql_f, false); //!REPARAR ESTA CONSULTA SQL
            //echo $sql; die();
            // core::preprint($_status,'status');exit();
            $data_f = Model::many_assoc($query_f[0]);
        ...
        $query_g = Database::ExeDoIt($sql_g, false); //!REPARAR ESTA CONSULTA SQL
        $data_g = Model::many_assoc($query_g[0]);
        //$x=0;
        //echo "**************************$x**************************<br>";
        ...
        group by NumPoliza, timerst";
            ...
            $query_h = Database::ExeDoIt($sql_h, false); //!REPARAR ESTA CONSULTA SQL
            //echo $sql; die();
            // core::preprint($_status,'status');exit();
            $data_h = Model::many_assoc($query_h[0]);
        ...
        for ($z = 0; $z < count($nombre); $z++) {
            ...
            $query_z = Database::ExeDoIt($sql_z, false); //!REPARAR ESTA CONSULTA SQL
            $data_z = Model::many_assoc($query_z[0]);
            if (count($data_z) == 0) {
    ...
    public static function mostrarTodasNotificacionesSiniestros()
    { //utilizado por notificaciones
        $sql = "SELECT id_notificaciones, nombre, reporte, siniestro, poliza, reserva, cantidad, detalle, leida, 
        estado, fecha_efectiva, fecha_actualizacion, fecha_registro FROM notificaciones
        where estado = 'A'
        and fecha_efectiva <= '" . date("Y-m-d H:i:s") . "'
        order by leida, nombre, reporte, siniestro, poliza, reserva, cantidad, detalle asc";
        $query = Database::ExeDoIt($sql, false); //!REPARAR ESTA CONSULTA SQL
        $data = Model::many_assoc($query[0]);
        return $data;
    }

    public static function mostrarTodasNotificacionesSiniestrosActivos()
    { //utilizado por notificaciones
    ...
    public static function actualizarNotificacionSiniestro($id)
    { //utilizado por notificaciones
        $sql = "update notificaciones set leida = 'Y' where
        estado = 'A'
        and fecha_efectiva <= '" . date("Y-m-d H:i:s") . "'
        and id_notificaciones = $id";
        $query = Database::ExeDoIt($sql, false); //!REPARAR ESTA CONSULTA SQL
        if ($query){
            return true;
        }
