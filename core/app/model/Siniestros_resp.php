...
class Siniestros{
    ...
    public static function verArchivosdelSiniestro($timerst,$areaId){
        $sql="SELECT s.*,lpad(s.version,2,'0') version,LOWER(c1.valor) c1,LOWER(c2.valor) c2,LOWER(c3.valor) c3 FROM `siniestros_files` s
        ...
        where s.timerst = '".$timerst."' and estatus = 1 and (s.area='$areaId' or s.area=(select id from areas where area='$areaId' limit 1 ) ) order by fecha DESC";
        $query = Database::ExeDoIt($sql,false);
        $data = Model::many_assoc($query[0]);
        // $data['info']= $sql;
        return $data;
    ...
    public static function segundaPartePreliminar($dataForm){ //guarda informe preliminar desde todos los siniestros
        ...
        // core::preprint($info,'info preliminar');exit();
         //obtener versión de formato.
         $sqlv= "SELECT version from siniestros_files where timerst='".$info['timerst']."' and nombre='informePreliminar' and estatus = 1 and area='".$info['areaID']."' order by id desc limit 1;";
         $query = Database::ExeDoIt($sqlv);
         $version= '1';
         if($query[0]->num_rows){
             $version= Model::many_assoc($query[0])[0]['version'];
...
Observaciones:
    ...
    public static function guardarInformeActualizacion($timerst,$data,$edit=false){ //guarda primera atencion desde todos los siniestros
        ...
        //obtener version de formato.
        $sqlv= "select version from siniestros_files where timerst='".$data['timerst']."' and nombre='informeActualizacion' estatus = 1 and area='".$data['areaID']."' order by id desc limit 1;";
        $query = Database::ExeDoIt($sqlv);
        $version= '1';
        if($query[0]->num_rows){
            $version= Model::many_assoc($query[0])[0]['version'];
    ...
    public static function guardarInformeCancelacion($timerst,$data){ //guarda informe de cancelacion desde todos los siniestros
        ...
        //obtener version de formato.
        $sqlv= "select version from siniestros_files where timerst='".$data['timerst']."' and nombre='informeCancelacion' and estatus = 1 and area='".$data['areaID']."' order by id desc limit 1;";
        $query = Database::ExeDoIt($sqlv);
        $version= '1';
        if($query[0]->num_rows){
            $version= Model::many_assoc($query[0])[0]['version'];
