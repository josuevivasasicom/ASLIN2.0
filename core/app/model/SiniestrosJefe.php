...
class SiniestrosJefe{
    ...
    public static function verArchivosdelSiniestro($timerst,$areaId){
        ...
        //este fragmento de codigo provocaba 
    ...
    public static function verSiniestroTimerst($timerst){
        ...
        LEFT JOIN config_campos cc on cc.id = s.calificacion as and cc.estado = 'A'
    ...
    public static function datosGraficasIndexCalificaciones(){
        ...
        //retorna el array de json`s con los datos de cada proveniente para contar todos los siniestros y generar las graficas.
        $sql= "SELECT count(s.id)cantidad, (select valor from config_campos cc where cc.id = s.calificacion and cc.estado = 'A' )calificacion FROM `siniestros` s where s.estado1 = 'A' GROUP by calificacion;";
		$query = Database::exeDoIt($sql);
    ...
    public static function datosGraficasIndexStatus(){
        ...
        //retorna el array de json`s con los datos de cada proveniente para contar todos los siniestros y generar las graficas.
        $sql= "SELECT count(s.id)cantidad, (select valor from config_campos cc where cc.id = s.status )`status` FROM `siniestros` s where s.estado1='A' GROUP by status;";
		$query = Database::exeDoIt($sql);
    ...
    public static function countTodosSiniestros($prov='',$statusS='',$area='',$asignados=false){// utilizado para jtabla siniestros  //?OPTIMIZADO PARA VARIOS USUARIOS
		...
		if($prov!='' and $statusS!=''){
            ...
            where
            sa.area ='".$area."' ";
            if($prov!='')   $sql.= " and s.f_numProv = (select id from config_prov where proveniente = '".$prov."' )";
            if($statusS!='')$sql.= " and status = '$statusS'";
            $sql.= " and s.estado='A' ";
        }
    ...
    public static function getTodosSiniestros($orden,$limitInf,$limitSup,$_prov='',$_status='',$area,$asignados=false){// utilizado para jtabla siniestros para abogadops
        ...
        FROM ". self::$tabla ." as s ";//siniestros
        ...
        $sql.= " LEFT JOIN - c on s.institucion = c.id ";
