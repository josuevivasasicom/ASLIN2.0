                    ...
                    <div class="col-md-12">
                        <div id="accordion">
                            <div class="card">
                                ...
                                <div class="card-header pt-0 " id="headingtwo" style="display:none;">
...
<script type="text/javascript" defer>
    ...
    if (localStorage.getItem('areas') == undefined) { 
        ...
        localStorage.setItem('areas', '<?=$areaIDuser?>');
    }
    ...
    localStorage.setItem('soloAsignados', false); */
        ...
        function leerTablaSiniestros() {
            ...
            if (localStorage.getItem('abogados') !='' ){
               /*  let x = JSON.parse( $('#abogados').val());
                $('#abogados').select2().value(x).trigger(); */

            }

            paramAbo = '&aboSelected=' + localStorage.getItem('abogados');

            paramProvAjax = paramProv + paramStatus + paramArea + soloAsignados + paramCalificacion + paramAbo; //proveniente seleccionado desde los botones
           
            paramNombres = nombreProv.toUpperCase()+' '+nombreStatus+' '+nombreCalificacion+' '+nomArea+' '+nombreAsign;
            
            console.log('paramProvAjax::');
            console.log(paramProvAjax);
            console.log('::paramProvAjax');

            $('#tablaSiniestrosTodos').jtable({
                title: 'Lista de Todos los Siniestros '+paramNombres,
                messages:{
                ...
                actions: {
                    listAction: '?action=jtable/tablaSiniestrosTodosJefe.ajax'+paramProvAjax,
                },
                fields: {
    ...
    window.onload=function (){
       ...
       /*  $("#btn_prov_F").on('click',function(){ //boton de filtro, al desplegar panel, esconde botones de provenientes
        ...
        /*$('#abogados').on( "change", function(event) {//hjace referencia al select2JS de abogados
            ...
            localStorage.setItem('abogados', JSON.stringify( $('#abogados').val() )  ); // en string

            $('#abogados').val( localStorage.setItem('abogados') ).trigger('change.select2');
    ...
    function selectStatus(statusS = '') {
        ...
        $('#tablaSiniestrosTodos').jtable('destroy');
        console.log("cargando tabla de status" + statusS);
