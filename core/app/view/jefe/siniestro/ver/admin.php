...
if ($k['verificado'] == 'Verificado' && $power==1) {                                //es admin y este item ya esta verificado
    $col.='<td style="text-transform:lowercase;" > '.$k['verificado'].' <small style="font-weight:500;" class="text-primary">'.$k['fecha_verificacion'].'</small> </td>';
}else if ($k['verificado'] == ' ' && $power==1) {  
    if($permisoKey[$_area]==true){                                 //es admin y mostrara el boton de verificar, 
        $col.='<td style="text-transform:lowercase;" >
        <button onclick="disparadorVerificar( \''.$sn['timerst'].'\'  ,\''.$k['id'].'\' )" class="btn btn-primary btn-sm mb-1" >  <i class="nc-icon nc-check-2"></i>  </button>
        ...
        $col.='<td style="text-transform:lowercase;" >
        <button style="'.$oculto.'" class="'.$notouchCLS.' btn btn-primary btn-sm" >  <i class="nc-icon nc-check-2"></i>  </button>
        </td>';
    }
}
