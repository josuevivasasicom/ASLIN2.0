...
<div class="modal fade" id="modalAsignarArea" tabindex="-1" role="dialog" aria-labelledby="modalAsignarAreaLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-primary" data-dismiss="modal" >Cerrar</button>
        <button type="button" class="btn btn-secondary"  onclick="this.disabled=true;asignArea()" >Asignar</button>
      </div>
    </div>
  </div>
</div>
...
<!-- INICIAN BOTONES DE SEGMENTACION POR AREAS SOLO PARA ADMINISTRADOR DE SINIESTROS Y ADMIN DE APP -->
<?php 
...
foreach ($sn['areasAsignadas'] as $k) {
  ...
  $oculto = ' '; if($_AreaID=='4' || $_AreaID==5){ $oculto = 'display:none !important;'; } //si es area 4 o 5 , cntabilidad o administrativa, oculta.
                              ...
                              if (count($files_sn)>=1){
                                ...
                                foreach ($files_sn as $k) {
                                  ...
                                  if ($render == 'si') {//check para generar link, si es formato gmx no lo genera.
                                    $nomAreaFile = $_area;
                                    if ($_area == 'ServidoresPúblicos') $nomAreaFile = 'Servidores Publicos'; //quitar acento
                                    // $col.='<td> </td>';/**************************************g*********************************************************************************************]/
                                  ...
                                  '<td style="text-transform:capitalize !important;" >'.$k['version'].'</td>'.
                                  '<td style="text-transform:capitalize !important; text-align:left;">'.$k['fecha'].'</td>'; 
                                  if ($render == 'si') {//necesita renderizar documento,
                                  $col.='<td style="text-transform:lowercase;" ><button onclick="vierFileF(true, \''.$_AREA.'\'  , \''.$sn['timerst'].'\'  ,\''.$k['nombre'].'\',  \''.$k['c1'].'\' , \''.$k['id'].'\' , \''.$k['version'].'\'  , \''.$k['c1'].'\' ,\''.$k['c2'].'\' ,\''.$k['c3'].'\'  )" class="btn btn-small btn-outline-primary" data-toggle="modal" data-target="#modalFileView"><i class="fa fa-eye" aria-hidden="true"></i> </button></td>'.
                                    '</tr>';
                                  }else{
                                    //no necesita renderizar, baja el archivo tal cual
                                    $col.='<td style="text-transform:lowercase;" ><button onclick="vierFileF(false, \''.$_AREA.'\'  , \''.$sn['timerst'].'\'  ,\''.$k['url'].'\',  \''.$k['c1'].'\' , \''.$k['id'].'\' , \''.$k['version'].'\'  , \''.$k['c1'].'\' ,\''.$k['c2'].'\' ,\''.$k['c3'].'\' )" class="btn btn-small btn-outline-primary" data-toggle="modal" data-target="#modalFileView"><i class="fa fa-eye" aria-hidden="true"></i> </button></td>'.
                                    '</tr>';
                                  }
                                }
                                print $col;
...
<!-- TERMINA EXPAND DE BOTONES DE SEGMENTACION SOLO PARA ADMINISTRADOR DE SINIESTROS Y ADMIN DE APP -->
      ...
      <div class="col-lg-12  pl-lg-0 pr-lg-0">
        <div class="card">
          ...
          <div class="card-body">
                <button onclick="vierFileCaratula('<?=$sn['timerst']?>','<?=$sn['folio']?>')" class="btn btn-primary" data-toggle="modal" data-target="#modalFileCaratula">caratula</button></td>
                ...
                <button style="float: right;" class="btn btn-secondary btn-special" onclick="window.location.href = '.?view=siniestro/editar&timerst=<?=$sn['timerst']?>';" >EDITAR ID</button>
                <div class="table">
                  <table class="table table-siniestro">
                    ...
                    <tr>
                      ...
                      <tr>
                          ...
                          <td>
                            <label>Status:</label>
                            <span name="Status" value="<?=$sn['status']?>" type="text" class="form-control <?=strtolower($sn['status'])?>" placeholder="Status" required> <?=$sn['status']?> </span>
                          </td>
                          <td>
                            <label>Calificación:</label>
                            <span name="calificacion" value="<?=$sn['calificacion']?>" type="text" class="form-control <?=strtolower($sn['calificacion'])?>" placeholder="Calificación" required> <?=$sn['calificacion']?> </span>
                          </td>
                      ...
                      <td>
                            ...
                            <span name="numReporte" value="<?=$sn['numReporte']?>" type="text" class="form-control" placeholder="Nº de Reporte" required> <?=$sn['numReporte']?> </span>
                          </td>
                          <td>
                            <label>Num Siniestro:</label>
                            <span name="numSiniestro" value="<?=$sn['numSiniestro']?>" row=2 type="text" class="form-control" placeholder="Nº de Siniestro" required> <?=$sn['numSiniestro']?> </span>
                          </td>
                          ...
                          <label>Fecha Vidgencia 1:</label>
                            <span name="fechaVigencia1" value="<?=$sn['vigencia1']?>" id="fechaVigencia1" type="text" class="form-control mx-0 px-0" placeholder="Fecha Vigencia" required> <?=$sn['vigencia1']?> </span>
                          </div>

                          <div class="col-6 mx-0 px-0" style="max-width:11em">
                            <label>Fecha Vigencia 2:</label>
                            <span name="fechaVigencia2" value="<?=$sn['vigencia2']?>" id="fechaVigencia2" type="text" class="form-control mx-0 px-0" placeholder="Fin Vigencia" required> <?=$sn['vigencia2']?> </span>
                            </div>
                          </div>
                          </td>
                      </tr>
                  ...
                  <div class="row">  <!-- solo se ve por lic tere -->
                      ...
                      <div class="col-6">
                          <div class="form-group">
                              ...
                              <span name="tipoIntervencion" value="" type="text" class="form-control" placeholder="" opcional XD> <?=$sn['tipoIntervencion']?> </span>
                          </div>
                      </div>
                      <div class="col-6">
                          <div class="form-group">
                              <label for="floatingInput">Tercero</label>
                              <span name="tercero" value="" type="text" class="form-control" placeholder="" opcional XD> <?=$sn['tercero']?> </span>
                          </div>
                      </div>
                      ...
                      <div class="col-6">
                          <div class="form-group">
                              ...
                              <span name="nicho" value="" type="nicho" class="form-control" placeholder="" opcional XD> <?=$sn['nicho']?> </span>
                          </div>
                      </div>
                      <div class="col-6">
                          <div class="form-group">
                              <label for="floatingInput">Materia</label>
                              <span name="materia" value="" type="materia" class="form-control" placeholder="" opcional XD> <?=$sn['materia']?> </span>
                          </div>
                      </div>
                      <div class="col-6">
                          <div class="form-group">
                              <label for="floatingInput">Expediente</label>
                              <span name="expediente" value="" type="expediente" class="form-control" placeholder="" opcional XD> <?=$sn['expediente']?> </span>
                          </div>
                      </div>
    ...
    <div class="col-lg-3 col-sm-12 pl-md-0">
                          ...
                          <div class="row" pol<?=$p['id']?> >
                              <div class="col-md-9 col-9">
                                <span class="text-muted"><small>No°: </small><b poliza><?=$p['poliza']?></b></span>
                                  <br>
                                  <span class="text-muted">Deducible: <small deducible> $ <?=number_format($p['deducible'], 0);?> </small></span> <br>
                                  <span class="text-muted">Reserva: <small reserva> $ <?=number_format($p['reserva'], 0);?> </small></span> <br>
                                  <span class="text-muted">Coaseg.: <small coaseguro> $ <?=number_format($p['coaseguro'], 0);?> </small></span> <br>
                                  <span class="text-muted">Sum Aseg.: <small sumaAsegurada > $ <?=number_format($p['sumaAsegurada'], 0);?> </small></span> <br>
                              </div>
                              <div class="col-md-3 col-3 text-right">
      ...
      <div class="col-md-12">
        <div class="card">
          <div class="card-header"><div class="card-title"><h5>Cambiar</div></div>
            <div class="card-body">
                <!-- Button trigger modal -->
                <div class="col-12">
                  <button type="button" onclick="changeParam('principal','status','<?=$sn['folio']?>')" class="pl-2 btn btn-primary btn-md" data-toggle="modal" data-target="#modalCambiaStatus">Status</button>
                </div>

                <div class="col-12">
                  <button type="button" onclick="changeParam('principal','calificacion','<?=$sn['folio']?>')" class="pl-2 btn btn-primary btn-md" data-toggle="modal" data-target="#modalCambiacalificacion">Calificacion</button>
                </div>
