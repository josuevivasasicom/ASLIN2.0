                            ...
                            <div class="card">
                                ...
                                <div class="form-group col-12">
                                    <select class="js-select2 form-control" placeholder="Buscar por Abogado" id="abogados" name="abogados" mNOultiple="multiple" style="float:right;width:85%;">
                                        ...
                                        //print "<option value='0'>Seleccione un abogado</option>";
...
<script type="text/javascript" defer>
    ...
    function leerTablaSiniestros() {
        ...
        if (localStorage.getItem('calificacion') != '') {
            ...
            paramCalificacion = '', nombreCalificacion = '';
            $($('[calificacion]')[0]).addClass('active');
            $($('[calificacion]')[0]).addClass('btn-link');
        }
        ...
        if (localStorage.getItem('prov') != '') {
            ...
            paramProv = '';
            nombreProv = '';
            $($('[prov]')[0]).addClass('active');
            $($('[prov]')[0]).addClass('btn-link');
        }
        ...
        if (localStorage.getItem('areas') != '') {
            ...
            paramArea = '';
            nomArea = '';
            $($('#headingtwo [areas]')[0]).addClass('active');
            $($('#headingtwo [areas]')[0]).addClass('btn-link');
        }
        ...
        if (localStorage.getItem('abogados') != '') {
            ...
            setTimeout(() => {
                ...
                //console.log("valor dinamic: " + dinamic);
                if (dinamic > 0){
                $('#abogados').select2("val", dinamic).trigger('change');
                
                // Set option selected onchange
                  // Set selected 
                    //$('#abogados').val(dinamic).trigger('change');
    ...
    function laveSelectAbogado() {
        ...
        localStorage.setItem('soloAsignados', "");
        //$('#abogados').select2().val([]).trigger();
        
        //document.querySelector('#btn1').click();
        let dinamic = 0;
        //$('#abogados').select2("val", ).trigger('change');
        //$("#abogados").select2().val("0").trigger("change");
        //$('#abogados').val(1).trigger('change');
        $("#prov").click();
        $("#status").click();
        $("#calificacion").click();
        $("#area").click();
        $("#siniestrosAsignados").prop("checked", false);

        //$('#abogados').val('0'); // Select the option with a value of '1'
        //$('#abogados').trigger('change'); // Notify any JS components that the value changed
...
<style>
    ...
    div.jtable-main-container>table.jtable>tbody>tr.jtable-data-row>td:nth-child(7) {
        /* status */
    ...
    div.jtable-main-container>table.jtable>tbody>tr.jtable-data-row>td:nth-child(9) {
        /* prov  */
        text-align: center;
    }

    div.jtable-main-container>table.jtable>tbody>tr.jtable-data-row>td:nth-child(8),
    /* folio, reporte  */
    ...
    div.jtable-main-container>table.jtable>tbody>tr.jtable-data-row>td:nth-child(14),
