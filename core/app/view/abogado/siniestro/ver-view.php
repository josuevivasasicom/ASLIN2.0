...
<!-- INICIAN BOTONES DE SEGMENTACION POR AREAS SOLO PARA ADMINISTRADOR DE SINIESTROS Y ADMIN DE APP2 -->
<?php 

$power=false;

foreach ($sn['abogadosAsignados'] as $abo) {
  //echo "->U" . $abo['usuario'] . "=S" . $_SESSION['id'] . "<-<br>";
...
foreach ($sn['areasAsignadas'] as $k) {
  ...
  //echo "Areas: " . $k['area'] . "<br>";
  $_AreaID= $k['id']; //id del area
  $_AREA = $k['area']; // nombre del area
  $_area = str_replace(' ','',$_AREA); //nombre del area sin espacios
  $notouchCLS=" ";$oculto = ' '; 
  //echo "power: " . $power . "<br>";
  //echo "_AreaID: " . $_AreaID . "<br>";
  //echo "permisoKey[$_area]: " . $permisoKey[$_area] . "<br>";
  if($power === false || ($_AreaID=='4' || $_AreaID == '5'  || $permisoKey[$_area] === false) ){ $notouchCLS=" notouch "; $oculto='display:none !important; pointer-events:none;'; } //si es area 4 o 5 , cntabilidad o administrativa, oculta.
...
<script type="application/javascript">
	$(document).ready(function () {
		$.noConflict();
		/*$('#theTable<?=$_area?>').DataTable({
			language: {
				url: 'https://cdn.datatables.net/plug-ins/1.13.4/i18n/es-MX.json',
				//cdn.datatables.net/plug-ins/1.13.4/i18n/es-MX.json
			},
			ajax: './?action=siniestro/datos&metodo=traer&param1=<?php echo $sn['timerst'] ?>&param2=<?php echo $_AreaID ?>&param3=<?php echo $_area ?>',
			columns: [
				{ "data": "blanco" },
				{ "data": "instancia_procedimiento" },
				{ "data": "etapa" },
