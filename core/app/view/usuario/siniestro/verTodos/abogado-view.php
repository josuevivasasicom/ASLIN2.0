                            ...
                            <div class="card">
                                ...
                                <!-- <button type="button" onclick="document.location.reload()" class="btn btn-primary  btn-round mr-3 btn_prov" ><b>Ver: Todas</b></button> -->
                                           ...
                                           foreach ($status as $key => $value) {
                                               echo ' <button type="button" onclick="selectStatus(\''.$value->id.'\')" status="'.$value->id.'" class="btn btn-primary btn-link btn-round btn_prov" >'.strtoupper($value->valor).'</button>';
                                            }
                                           ?>
                                <!-- fin row2 -->
                                </div>
...
<script type="text/javascript" defer>
    ...
    function leerTablaSiniestros(paramProv,paramStatus){
        mProvNombre= "todos los Provenientes";
        paramProvAjax='';
        if(paramProv!='' || paramStatus!='' ){ //determina el parametro a utilizar segun el proveniente seleccionado
            paramProvNombre=paramProv.toUpperCase(); //proveniente
            paramProvAjax='&provSelected='+paramProv+'&statusSelected='+paramStatus; //proveniente seleccionado desde los botones
            console.log('parametros diferentes');
            console.log(paramProvNombre);
        }
        paramProvNombre = paramProv + paramStatus; //limpia el nombreS
    ...
    function selectProv(prov){ //selecciona el provedor a ver en la tabla.
       ...
       let statusS = '';
       if(localStorage.getItem('statusS') === 'undefined' || localStorage.getItem('statusS') === '' ){ 
            console.log('sin valor');
            statusS = '';
            $('[status=todas]').addClass('active');
            $('[status=todas]').addClass('btn-link');
        }
        else{
            statusS = localStorage.getItem('statusS');
            $('[status='+statusS+']').addClass('active');
            $('[status='+statusS+']').addClass('btn-link');
        }
        leerTablaSiniestros(prov,statusS);
    }
    ...
    function selectStatus(statusS){
       ...
       if(localStorage.getItem('prov') === 'undefined'){ 
        ...
        $('#tablaSiniestrosTodos').jtable('destroy');
        console.log("cargando tabla de "+prov+" con "+statusS);
        localStorage.setItem('statusS',statusS);
        if(localStorage.getItem('statusS') !== 'undefined'){ 
            let statusS = '';
        }else{
            let statusS = localStorage.getItem('prov');
        }
        leerTablaSiniestros(prov,statusS);
    }
