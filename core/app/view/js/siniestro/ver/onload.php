...
window.onload = function() {
    ...
    $('.navbar-brand').html(
        "ID: <b><?= $sn['folio'] ?></b> -  Num siniestro: <b><?= $sn['numSiniestro'] ?> <br> <?= $sn['nombre'] ?> <?= $sn['apellidoP'] ?> <?= $sn['apellidoM'] ?>"
    );
    $('.sidebar-wrapper li[siniestros]').addClass(
        'active'); //activa el menu de siniestros  <?= $sn['numSiniestro'] ?>
    ...
    function formatState(state) {
        ...
        var $state = $(
            ...
            '" class="img-flag" style="display: inline-block; background: transparent !important;width: 2em;height: auto;border-radius: 18% !important;margin-right: 0.6em !important;" /> ' +
            state.text + '</span>'
                ...
                success: function(respuesta) {
                    ...
                    if (respuesta) {
                        ...
                        respuesta.forEach(abogado => {
                            $("<option avatar='" + abogado.avatar + "' value='" +
                                abogado.id + "'>" + abogado.nombre + " <i>: " +
                                abogado.area + "</i> </option>").appendTo($(
                                "#abogados"));
                        });
                        $('#abogados').select2({
                            placeholder: $('#abogados').attr('placeholder'),
                ...
                success: function(respuesta) {
                    ...
                    if (respuesta.siniestro) {
                        ...
                        Swal.fire({
                            ...
                            html: 'Siniestro <a href="./?view=siniestro/ver&siniestro=' +
                            ...
                            if (result.isConfirmed) {
                                ...
                                window.location = "./?view=siniestro/ver&siniestro=" +
                                    respuesta.siniestro;
    ...
    console.log($('#tablaHistoricoSiniestros').jtable('load'));
...
function cambiaNombre() {
    ...
    Swal.fire({
        ...
        html: `
        <input letter="capitalize"  style="width:100%;text-align: center; text-transform:capitalize;" type="text" name="nombreAsegurado" id="nombreAsegurado" value="` +
            "<?= $sn['nombre'] ?>" +
            `"> 
        <input letter="capitalize"  style="width:100%;text-align: center; text-transform:capitalize;" type="text" name="paternoAsegurado" id="paternoAsegurado" value="` +
            "<?= $sn['apellidoP'] ?>" +
            `"> 
        <input letter="capitalize"  style="width:100%;text-align: center; text-transform:capitalize;" type="text" name="maternoAsegurado" id="maternoAsegurado" value="` +
            "<?= $sn['apellidoM'] ?>" + `"> 
        `,
        didOpen: () => {
...
//Eliminar archivo del siniestro 

function deleteFile() {
    // Obtener el ID y timerst del archivo que se desea eliminar
    var idArchivo = $('#idfile').val();
    var timerstArchivo = $('#timerstFile').val();

    // Enviar la solicitud AJAX utilizando jQuery
    $.ajax({
        url: './?action=siniestro/eliminar&method=eliminarfile/ver&id=' + idArchivo + '&timerst=' +
            timerstArchivo,
        method: 'GET',
        data: {},
        success: function(response) {
            ...
            Swal.fire(`
                        Cambios realizados 
                    `.trim()).then(() => {
                window.location.href = '?view=siniestro/ver&param=' + timerstArchivo;
                //actualiza la pagina al hacer click en ok, para visualizar los cambios.
            })
        },
        error: function() {
    ...
    $.ajax({
        ...
        success: function(respuesta) {
            ...
            dataset.forEach(acount => {
                ...
                listTD = listTD + `
                <tr>
                <td> <input type="checkbox" ` + check + ` name="send_` + acount.idlist + `" id="send_` + acount
                    .idlist + `"  ` + disabled + `> <td>
                <td> <input type="checkbox" ` + check2 + ` name="cc_` + acount.idlist + `" id="cc_` + acount.idlist +
                    `"     ` + disabled + `> <td>
                <td> <input type="checkbox" ` + check3 + ` name="cco_` + acount.idlist + `" id="cco_` + acount.idlist +
                    `"   ` + disabled + `> <td>
                <td>` + acount.email + `<td>
                <td>` + acount.nombre + `<td>
            ...
            Swal.fire({
                ...
                html: "<style>.tablaEmails tr:nth-child(4n+1){background:#e1e9ed;} .tablaEmails tr:hover{background: #ffedc3b3;color: black;}.tablaEmails tr:nth-child(1){background:#fff;} </style> <table width='100%' class='tablaEmails'>" +
                    listTD + "<table> <br>" + addmails,
                didOpen: () => {
                    //select2 de mas emails
                    $('#inputsend').select2({
                        placeholder: $('#inputsend').attr(
                ...
                preConfirm: () => {
                    ...
                    const save_send = Swal.getPopup().querySelector(
                        '[type=checkbox][name=save_send]').checked;
                    const save_cc = Swal.getPopup().querySelector(
                        '[type=checkbox][name=save_cc]').checked;
                    const save_cco = Swal.getPopup().querySelector(
                        '[type=checkbox][name=save_cco]').checked;
                    ...
                    const send = Swal.getPopup().querySelectorAll(
                        '[type=checkbox][name^=send_]:checked');
                    const cc = Swal.getPopup().querySelectorAll(
                        '[type=checkbox][name^=cc_]:checked');
                    const cco = Swal.getPopup().querySelectorAll(
                        '[type=checkbox][name^=cco_]:checked');
                    let sendMails = ' ';
                    let ccMails = ' ';
                    let ccoMails = ' ';
                    send.forEach(item => {
                        sendMails = sendMails + "'" + item.id + "',"
                    });
                    cc.forEach(item => {
                        ccMails = ccMails + "'" + item.id + "',"
                    });
                    cco.forEach(item => {
                        ccoMails = ccoMails + "'" + item.id + "',"
                    });
                    ...
                    if (sendMails == '')
                        Swal.showValidationMessage(
                            `No se puede enviar mail sin un destinatario.`);
                            ...
                            $.ajax({
                                ...
                                success: function(respuesta) {
                                    ...
                                    if (respuesta == 'EXITO') {
                                        ...
                                        texto =
                                            "No se logró enviar, posiblemente el archivo es muy pesado";
                                    } else {
                                        texto =
                                            "OPS!! algo salió mal. informa al desarrollador";
                                    }
...
function editPoliza(i) {
    ...
    Swal.fire({
        ...
        html: `
          <form class="swetForm" style="overflow-x:hidden">
            ...
            <div class="form-group row">
              <label for="poliza" class="col-sm-4 col-form-label">No. Poliza</label>
              <div class="col-sm-8">
                <input type="text" class="form-control" id="numPoliza" name="numPoliza" placeholder="No. Poliza" value="` +
            pol
            .poliza +
            `">
              </div>
            ...
            <div class="form-group row">
              ...
              <div class="col-sm-8">
                <input type="text" class="form-control" id="deducible" name="deducible" placeholder="Deducible" value="` +
            pol
            .deducible + `">
              </div>
            </div>
            ...
            <div class="form-group row">
              ...
              <div class="col-sm-8">
                <input type="text" class="form-control" id="reserva" name="reserva" placeholder="Reserva" value="` +
            pol.reserva +
            `">
              </div>
            </div>
            ...
            <div class="form-group row">
              ...
              <div class="col-sm-8">
                <input type="text" class="form-control" id="coaseguro" name="coaseguro" placeholder="coaseguro" value="` +
            pol
            .coaseguro +
            `">
              </div>
            ...
            <div class="form-group row">
              ...
              <div class="col-sm-8">
                <input type="text" class="form-control" id="sumaAsegurada" name="sumaAsegurada" placeholder="Número" value="` +
            pol
            .sumaAsegurada + `">
              </div>
            </div>
        ...
        didOpen: () => {
            ...
            $('#numPoliza').on('input', function(event) {
            ...
            var mony = {
                ...
                blocks: {
                    num: {
                        ...
                        thousandsSeparator: ',',
                        radix: '.',
                        padFractionalZeros: true, // Agregar esta propiedad y establecerla en true
                        // Agregar un filtro personalizado para agregar ".00" al final de los valores
                        filter: function(value) {
                            var suffix = '.00';
                            // Si no hay decimales en el valor, se agrega ".00" al final
                            //if (value.indexOf('.') === -1) {
                            return value + suffix;
                            //}
                            // Si el valor tiene decimales pero no tiene los dos dígitos después del punto decimal, se agrega ceros adicionales
                            /*var decimalPart = value.split('.')[1];
                            if (decimalPart.length === 1) {
                                return value + '0';
                            }*/
                            //return value; // El valor ya tiene los dos dígitos después del punto decimal, no se agrega nada
                        }
                    }
                }
            };

            var deducibleMask = IMask(document.getElementById('deducible'), mony);
            var reservaMask = IMask(document.getElementById('reserva'), mony);
            var coaseguroMask = IMask(document.getElementById('coaseguro'), mony);
            var sumaAseguradaMask = IMask(document.getElementById('sumaAsegurada'), mony);
                    ...
                    $.ajax({
                        ...
                        success: function(respuesta) {
                            ...
                            if (respuesta == 'ok') {
                                ...
                                $(".row[pol" + d.id + "] [sumaAsegurada]").html(d
                                    .sumaAsegurada);
                                ...
                                Swal.fire({
                                    ...
                                    title: '¡Datos actualizados!',
                                    // text: 'El cliente puede pasar a caja a pagar con su nombre o puedes seguir editando.',
                                    html: "Cantidades actualizadas, se procede a cargar la visualización de los nuevos datos",
                                    confirmButtonText: 'OK',
                                    allowOutsideClick: false
                                }).then((result) => {
                                    location.reload();
...
function disparadorFile(area) {
    ...
    let options = '';
    // console.log("Hola tu: ");
    // console.log(dataFilesSelect);

    dataFilesSelect.forEach(opt => {
        options = options + '<option value="' + opt.id + '"> [' + opt.c3 + '] <b>' + opt.c2 + '<b> - ' + opt
            .valor + '</option>';
    });

    let selInput = `<input type='hidden' value='` + area +
        `' name='area' /> <select name="tipo" id="tipo"  class="swal2-input select2 js-select2" placeholder="Selecciona la clasificación del archivo"  style="width:100%;">` +
        options + `</select>
    ...
    let html_ =
        ` <form id="formFile" action="./?action=siniestro/files&modo=upload" enctype="multipart/form-data" method="POST"> <input type="file" accept="pdf/doc/docx/csv/xlsx/xls" aria-label="Selecciona un archivo CSV de siniestros" name="importFile" id="importFile" class="swal2-file" placeholder="Importar Archivo" style="display: flex;">`;
...
function vierFileF(tipo, area, timerst, url, title, idfile, version, c1, c2, c3) {
    ...
    $("#modalFileViewBody").html('<iframe src="./?action=pdf/viewFileF&timerst=' + timerst + '&doc=' + url +
        '&areaNombre=' + area + '&v=' + version + '" frameborder="0" style="width:100%;height:66vh;"></iframe>');
    $('#linkpdf').val('https://www.claimsmanager.online/?action=pdf/viewFileF&timerst=' + timerst + '&doc=' + url +
        '&areaNombre=' + area + '&v=' + version);
    $('#link').val(url);
    $('#idfile').val(idfile);
    $('#version').val(version);
    $('#c1').val(c1);
...
function delFileF(idfile, timerst, name) {
    let title = 'Eliminar Archivo <strong>' + name + '</strong>';

    $('#modalFileDelLabel').html(title);

    $("#modalFileDelBody").html('Estas a punto de eliminar un archivo. <br /> <strong>' + idfile + ' / ' + timerst +
        '</strong>');
...
function vierFileD(tipo, area, timerst, url, title, idfile, version, c1, c2, c3) {
    $('#modalFileViewLabel').html(title);
    $("#modalFileViewBody").html('<iframe src="./?action=pdf/viewFileD&timerst=' + timerst + '&doc=' + url +
        '&areaNombre=' + area + '&v=' + version + '" frameborder="0" style="width:100%;height:66vh;"></iframe>');
    $('#linkpdf').val('https://www.claimsmanager.online/?action=pdf/viewFileF&timerst=' + timerst + '&doc=' + url +
        '&areaNombre=' + area + '&v=' + version + '&c3=' + c3);
    $('#link').val(url);
    $('#idfile').val(idfile);
    $('#version').val(version);
    $('#c1').val(c1);
...
function vierFileCaratula(timerst, folio) {
    ...
    $("#modalFileCaratulaBody").html('<iframe src="./pdf/viewFileF-direct.php?timerst=' + timerst +
        '&doc=caratula" frameborder="0" style="width:100%;height:66vh;"></iframe>');
    // $('#linkpdf').val('https://www.claimsmanager.online/?action=pdf/viewFileF&timerst='+timerst+'&doc='+url+'&areaNombre='+area+'&v='+version);
...
function createLinksFiles() {
    ...
    var Ainterval = setInterval(function() {
        ...
        if (aIndex < archivos.length) {
            ...
            console.log("aindex:" + archivos[aIndex]);
            ...
            } else if (file.indexOf("https") == 0) {
                //console.log("fileHost:"+ file);
                //file = file.split('&')[2];
                //console.log("fileHost:"+ file);
            }
            ...
            setTimeout(() => {
                ...
                if (file.indexOf("https") == 0) {
                    ...
                    file2 = file.split('&')[3];
                    let text1 = file1;
            let result1 = text1.replace("doc=", "");
            let text2 = file2;
            let result2 = text2.replace("areaNombre=", "");
                    $('#downloader').attr('download',result1+result2 +".pdf"); //? nombre del archivo*/
                    window.open(file, '_blank');
                    //console.log("descargando: " +result1+result2 +".pdf");
                } else {
...
function createLinksFilesForMail() {
    ...
    var Ainterval = setInterval(function() {
        ...
        if (aIndex < archivos.length) {
            ...
            let file = archivos[aIndex];
            if (file.split('./')[
                    1]) { //si se puede hacer split es por que trae punto y es fichero para renderizar
                file = file.split('./')[1];
                file = host + '' + file;
            } else {
            ...
            dhtml += `<li>
                    <a href="` + file + `" download="` + decodeURIComponent(decodeURI(name)) + `" link>` +
                decodeURIComponent(decodeURI(name)) + ` </a>
                  </li>`;
    ...
    function continuarmsg(dhtml) {
        ...
        emlCont += "<!DOCTYPE html><html><head></head><body style='background-color: white; color: black'>" + txt +
            "</body></html>";
        // console.log(emlCont) ;
        var textFile = null;
        var data = new Blob([emlCont], {
            type: 'text/plain'
        });
        if (textFile !== null) {
