...
class TablaSiniestrosBuscarAs{
  ...
  =============================================*/
  public function mostrarTablaResultadosSiniestrosProv($prov,$status,$area,$asignados,$abogados,$calificacion,$asegurado){ //SE EJECUTA POR DEFAULT
    $R = $_REQUEST;
    ...
    $query_total= Siniestros::countBusquedaTodosSiniestros($prov,$status,$area,$calificacion,$asignados,$asegurado); //?devuelve un string con el número
    ...
    //todos
        $datosSiniestros = Siniestros::getBusquedaTodosSiniestros($orden,$limitInf,$limitSup,$prov,$status,$area,$calificacion,$asignados,$asegurado);

        //obtener datos de usuario y validar qeu se genere la tabla desde JS

    $jTableResult = array();
    $jTableResult['Result'] = "OK";
    $jTableResult['data'] = "Resultados de busqueda para ".$asegurado;
...
if( 
  ...
  $searchAsegurado = strtolower($_REQUEST['searchAsegurado']);

  
  $activar -> mostrarTablaResultadosSiniestrosProv($provSelected, $statusSelected,$area,$asignados,$abogados=[],$calificacionSelected,$searchAsegurado);
